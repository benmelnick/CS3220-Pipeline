...
typedef struct {
    ...
    XMulbf16_uio_map maps[ MAX_UIO_MAPS ];
} XMulbf16_uio_info;

/***************** Variable Definitions **************************************/
static XMulbf16_uio_info uio_info;
...
static int uio_info_read_name(XMulbf16_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMulbf16_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMulbf16_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMulbf16_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMulbf16_Initialize(XMulbf16 *InstancePtr, const char* InstanceName) {
	XMulbf16_uio_info *InfoPtr = &uio_info;
